...
def locate(msg, user):
    ...
    last_seen = Times.objects.filter(device__in=u.devices.all()).order_by('-last_seen')[0].last_seen
    last_seen = naturaltime(last_seen)
    
    content = 'Last saw %(fred)s %(at_basecamp)s %(an_hour_ago)s' % dict(
        fred=user,
        at_basecamp=random.choice(['at basecamp', 'in the bat cave', 'in the office', 'sleeping at their desk', 'getting coffee', 'eating', 'hiding from you']),
